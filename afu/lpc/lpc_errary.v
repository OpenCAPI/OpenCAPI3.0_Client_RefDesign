...
//                               |-----------|     |-------- |---------------------
  `define LPC_ERRARY_VERSION     02_Feb_2017   //
// -------------------------------------------------------------------
...
// Also check for simultaneous load bits on, which is an error (make all fields same length to avoid Portals warning)
...
//   read repeatedly since the rdptr would not advance past the last valid entry. My guess is the latter would be
//   more confusing, so for now advance the pointer on all reads of the register. 
...
//  else if (ery_data_done == 1'b1 && rdptr != 3'b111 && errary_val[rdptr] == 1'b1)
